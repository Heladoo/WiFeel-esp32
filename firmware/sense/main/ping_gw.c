...
esp_err_t ping_gw_start(const esp_ip4_addr_t *target, uint32_t interval_ms)
{
    ...
    /* on_ping_success is the liveness signal ping_gw_ms_since_last_success()
     * reports on — see ping_gw.h. We don't hook on_ping_timeout: a dead
     * link has been observed to fail at the socket *send* itself (logged
     * directly by the ping_sock component, outside any of esp_ping's
     * callbacks), which doesn't necessarily ever reach a timeout callback
     * either. "Time since last success" catches both failure shapes. */
